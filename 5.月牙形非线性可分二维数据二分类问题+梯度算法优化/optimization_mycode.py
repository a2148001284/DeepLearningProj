...
def random_mini_batches(X, Y, mini_batch_size=64, seed=0):
    ...
    # 对所有数据进行打乱
    permutation = list(np.random.permutation(m))  #生成一个随机排列的整数列表

    # X[:, permutation]表示选取X的所有行，但是列的顺序按照permutation中的索引进行重新排列
    shuffled_X = X[:, permutation]  #X[:, i]表示选取二维数组X的所有行（第一个维度）和第i列（第二个维度）
    shuffled_Y = Y[:, permutation].reshape((1, m)) #将重新排列后的Y数组重塑为形状为(1, m)的二维数组

    # 循环将每批次数据按照固定格式装进列表当中
    num_complete_minibatches = math.floor(m / mini_batch_size) #总样本除以每批次的数据数量 得到批次的数量

    # 所有训练数据分成多少组
    #表示选取所有行（第一个维度），但只选取列的范围从k * mini_batch_size到(k + 1) * mini_batch_size的部分。这样就得到了一个小批次大小的特征矩阵
    ...
    # 最后剩下的样本数量mini-batch < mini_batch_size 表示剩下的数据不满足一个样本的时候 全部都加入最后一个样本
...
def model(X, Y, optimizer, learning_rate=0.0007, mini_batch_size=64, beta=0.9,
    ...
    for i in range(num_epochs):
        ...
        for minibatch in minibatches:
            (bach_X, bach_Y) = minibatch
            #前向传播
            y_predict , cache = forward_propagation(bach_X, parameters)
            #计算损失
            loss = compute_cost(y_predict,bach_Y)
            #反向传播计算梯度
            grad = backward_propagation(bach_X,bach_Y,cache)
            #梯度更新
            if optimizer == "momentum":
                parameters , v = update_parameters_with_momentum(parameters,grad,v,beta,learning_rate)
            elif optimizer == "adam":
                t = t + 1
                parameters, v, s = update_parameters_with_adam(parameters,grad,v,s,t,learning_rate,beta1,beta2,epsilon)

        # 结束

        # 每个1000批次打印损失
        if print_cost and i % 1000 == 0:
            print("第 %i 次迭代的损失值: %f" % (i, loss))
