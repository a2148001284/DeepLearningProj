...
def compute_cost_with_regularization(A3, Y, parameters, lambd):
    ...
    # 开始
    tmp_cost = lambd / (2 * m) * (np.sum(np.square(W1)) + np.sum(np.square(W2)) + np.sum(np.square(W3)))
    cost = tmp_cost + cross_entropy_cost
    # 结束
...
def model(X, Y, learning_rate=0.3, num_iterations=30000, lambd=0, keep_prob=1):
    ...
    for i in range(0, num_iterations):
        ...
        if keep_prob == 1: #为1表示不进行dropout正则化
        ...
        else:  #lambd不为0 则计算损失的时候采用L2正则化的方法
            cost = compute_cost_with_regularization(a3, Y, parameters, lambd)

        # 只允许选择一个，要么L2正则化，要么Droupout
        assert (lambd == 0 or keep_prob == 1)

        if lambd == 0 and keep_prob == 1: #初始情况
            grads = backward_propagation(X, Y, cache)
        elif lambd != 0:  #L2正则化的情况
